...
def _impact_level(shap_value: float) -> str:
    """Classify a SHAP contribution for the certificate UI.

    The threshold is deliberately kept in one place so the API, Streamlit
    card, and standalone HTML dashboard all describe severity consistently.
    """
...
def generate_certificate(attack_type: str, confidence: float, top_shap_values: list) -> dict:
    ...
    """
    # For an attack, emphasize evidence that pushed the classifier toward the
    # predicted class. For BENIGN, show the strongest available evidence so
    # analysts can see why the traffic was cleared.
    driving = [f for f in top_shap_values if f.get("impact") == "positive"]
    if attack_type == "BENIGN" or not driving:
        driving = top_shap_values

    reasons = []
    phrases = []
    for f in driving[:3]:
        phrase = _phrase_for(f.get("feature", "unknown feature"), f.get("impact", "negative"))
        phrases.append(phrase)
        reasons.append({
            "phrase": phrase,
            "impact_level": _impact_level(f.get("shap_value", 0.0)),
        })
